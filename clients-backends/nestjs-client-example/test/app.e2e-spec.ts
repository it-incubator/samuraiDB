...
describe('AppController (e2e)', () => {
  ...
  const FIRST_PORTION_MAX_ITEM_NUMBER = 40;

  it('/samurais (POST & PUT + DELETE + GET verification)', async () => {
    for (let i = 1; i <= FIRST_PORTION_MAX_ITEM_NUMBER; i++) {
  ...
  it('/samurais add new samurais after compaction', async () => {
    for (let i = FIRST_PORTION_MAX_ITEM_NUMBER + 1; i <= 300; i++) {
      // Каждые 3 шага — обновление рандомного самурая
      if (i % 3 === 0 && createdSamurais.length > 0) {
        const randomEntry =
          createdSamurais[Math.floor(Math.random() * createdSamurais.length)];
